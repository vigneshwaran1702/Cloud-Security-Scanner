...
export default function SubscriptionSettingsTab() {
  const { currentPlan, activeTier, isPro, isEnterprise, invoices } = useSubscription();
  const [activeSubTab, setActiveSubTab] = useState('details'); // 'details' | 'invoices' | 'features'
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 pb-2 border-b" style={{ borderColor: 'var(--border-color)' }}>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 ${
              activeSubTab === 'details'
                ...
                : 'text-muted hover:text-main hover:bg-panel-inner'
            }`}
            style={{
              background: activeSubTab === 'details' ? 'var(--primary)' : 'var(--panel-inner-bg)',
              color: activeSubTab === 'details' ? '#ffffff' : 'var(--text-muted)',
              border: activeSubTab === 'details' ? '1px solid var(--primary)' : '1px solid var(--border-color)',
            }}
          >
            <ShieldCheck size={16} />
          ...
          <button
            ...
            style={{
              background: activeSubTab === 'invoices' ? 'var(--primary)' : 'var(--panel-inner-bg)',
              color: activeSubTab === 'invoices' ? '#ffffff' : 'var(--text-muted)',
              border: activeSubTab === 'invoices' ? '1px solid var(--primary)' : '1px solid var(--border-color)',
            }}
          >
            <Receipt size={16} />
          ...
          <button
            ...
            style={{
              background: activeSubTab === 'features' ? 'var(--primary)' : 'var(--panel-inner-bg)',
              color: activeSubTab === 'features' ? '#ffffff' : 'var(--text-muted)',
              border: activeSubTab === 'features' ? '1px solid var(--primary)' : '1px solid var(--border-color)',
            }}
          >
            <Layers size={16} />
            Tier Safeguards Breakdown
          </button>
        </div>
      ...
      {activeSubTab === 'details' && (
        <div className="flex flex-col gap-6 animate-fade-in">
          ...
          >
            ...
            {/* Payment Method / Payment Summary Details */}
          ...
          {/* Quick Access to Invoices list preview */}
          {invoices.length > 0 && (
            <div className="glass-panel" style={{ padding: '24px' }}>
              <div className="flex justify-between items-center" style={{ marginBottom: '16px' }}>
                <div className="flex items-center gap-2">
                  <Receipt size={18} color="var(--primary)" />
                  <h4 style={{ fontSize: '1.05rem', margin: 0, fontWeight: 700 }}>Recent Verified Invoices</h4>
                </div>
                <button
                  type="button"
                  onClick={() => setActiveSubTab('invoices')}
                ...
                >
                  View All ({invoices.length}) <ChevronRight size={14} />
                </button>
              </div>
              ...
              <div className="flex flex-col gap-3">
                {invoices.slice(0, 2).map(inv => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div style={{ padding: '8px', borderRadius: '8px', background: 'var(--badge-primary-bg)', color: 'var(--primary)' }}>
                        <Receipt size={16} />
                      </div>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>
                          {inv.id} — {inv.planName || 'Pro Security Shield'}
                        </div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                          {inv.formattedDate || inv.date} • {inv.paymentMethodLabel || 'Credit Card'}
                        </div>
                      </div>
                    </div>
      ...
      {/* SUB-VIEW 2: INVOICES & BILLING HISTORY */}
      {activeSubTab === 'invoices' && (
